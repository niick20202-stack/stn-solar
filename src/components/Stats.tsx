import { useState, useEffect, useRef } from 'react'
import { motion } from 'framer-motion'

const testimonials = [
  {
    initial: 'M',
    name: 'Markus W.',
    location: 'Düsseldorf, NRW',
    text: '\"Von der Planung bis zum Netzanschluss war alles perfekt organisiert. Die Anlage läuft seit Tag 1 besser als erwartet!\"',
    rating: 5,
  },
  {
    initial: 'S',
    name: 'Stefan K.',
    location: 'Neuss, NRW',
    text: '\"In weniger als einer Woche war alles installiert und in Betrieb. Endlich unabhängig — der Speicher war die beste Entscheidung.\"',
    rating: 5,
  },
  {
    initial: 'L',
    name: 'Lisa M.',
    location: 'Köln, NRW',
    text: '\"Kein Konzernapparat, sondern echte persönliche Betreuung. Der Solarplan war in Minuten fertig — und genau so wie versprochen.\"',
    rating: 5,
  },
]

function Stars({ count = 5 }: { count?: number }) {
  return (
    <div className="flex gap-1">
      {Array.from({ length: count }).map((_, i) => (
        <span
          key={i}
          className="material-symbols-outlined icon-filled star-glow"
          style={{ color: '#F5C518', fontSize: '20px' }}
        >
          star
        </span>
      ))}
    </div>
  )
}

function CounterSection() {
  const [count, setCount] = useState(0)
  const ref = useRef<HTMLDivElement>(null)
  const hasRun = useRef(false)

  useEffect(() => {
    const el = ref.current
    if (!el) return
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !hasRun.current) {
          hasRun.current = true
          const target = 150
          const duration = 2200
          const start = performance.now()
          const tick = (now: number) => {
            const elapsed = now - start
            const progress = Math.min(elapsed / duration, 1)
            const eased = 1 - Math.pow(1 - progress, 4)
            setCount(Math.floor(eased * target))
            if (progress < 1) requestAnimationFrame(tick)
            else setCount(target)
          }
          requestAnimationFrame(tick)
        }
      },
      { threshold: 0.3 }
    )
    observer.observe(el)
    return () => observer.disconnect()
  }, [])

  return (
    <div ref={ref} className="flex flex-col items-center gap-2">
      <div
        className="font-black leading-none tracking-tighter"
        style={{
          fontFamily: 'DM Sans, sans-serif',
          fontSize: 'clamp(3.5rem, 12vw, 10rem)',
          letterSpacing: '-0.06em',
          color: '#1a1a1a',
        }}
      >
        <span style={{ fontFamily: 'DM Mono, monospace' }}>
          {count.toLocaleString('de-DE')}
        </span>
        <span className="text-gradient">+</span>
      </div>
      <p
        className="text-sm font-bold uppercase tracking-[0.4em] text-center"
        style={{ fontFamily: 'Space Grotesk, sans-serif', color: 'rgba(26,26,26,0.45)' }}
      >
        Installierte Anlagen in NRW
      </p>
    </div>
  )
}

export default function Stats({ onAngebot }: { onAngebot: () => void }) {
  return (
    <section
      id="feedback"
      className="relative overflow-hidden py-16 md:py-32 px-5 md:px-6"
    >
      {/* Ambient glows */}
      <div
        className="absolute -bottom-24 -left-24 w-96 h-96 rounded-full pulse-soft pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(245,176,64,0.05) 0%, transparent 70%)' }}
      />
      <div
        className="absolute top-24 right-0 w-80 h-80 rounded-full pulse-soft pointer-events-none"
        style={{
          background: 'radial-gradient(circle, rgba(245,176,64,0.04) 0%, transparent 70%)',
          animationDelay: '1.5s',
        }}
      />

      <div className="max-w-7xl mx-auto">
        {/* Social proof badges */}
        <motion.div
          className="flex justify-center mb-8 md:mb-12 reveal"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8 }}
        >
          <div
            className="flex flex-wrap justify-center items-center gap-6 px-8 py-4 rounded-full"
            style={{
              background: 'rgba(0,0,0,0.03)',
              border: '1px solid rgba(0,0,0,0.07)',
            }}
          >
            {[
              { icon: 'G', label: 'Google Reviews' },
              { icon: null, label: 'Trusted Shops', matIcon: 'verified_user' },
              { icon: null, label: 'Trustpilot', matIcon: 'star' },
            ].map((badge, i) => (
              <div key={badge.label} className="flex items-center gap-5">
                {i > 0 && (
                  <div className="w-px h-4" style={{ background: 'rgba(0,0,0,0.1)' }} />
                )}
                <div
                  className="flex items-center gap-2 opacity-40 hover:opacity-90 transition-opacity duration-300"
                >
                  {badge.icon ? (
                    <span
                      className="font-black text-lg"
                      style={{ fontFamily: 'DM Sans, sans-serif', letterSpacing: '-0.05em', color: '#1a1a1a' }}
                    >
                      {badge.icon}
                    </span>
                  ) : (
                    <span className="material-symbols-outlined text-sm" style={{ color: '#1a1a1a' }}>
                      {badge.matIcon}
                    </span>
                  )}
                  <span
                    className="text-[10px] font-bold uppercase tracking-widest"
                    style={{ fontFamily: 'Space Grotesk, sans-serif', color: '#1a1a1a' }}
                  >
                    {badge.label}
                  </span>
                </div>
              </div>
            ))}
          </div>
        </motion.div>

        {/* Counter */}
        <div className="text-center mb-10 md:mb-16 reveal">
          <CounterSection />
        </div>

        {/* Rating widget */}
        <motion.div
          className="flex justify-center mb-12 md:mb-20 reveal"
          initial={{ opacity: 0, scale: 0.95 }}
          whileInView={{ opacity: 1, scale: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.7 }}
        >
          <div
            className="glow-card flex flex-col items-center px-6 py-5 md:px-10 md:py-8 rounded-3xl"
            style={{
              background: 'rgba(0,0,0,0.03)',
              backdropFilter: 'blur(16px)',
              border: '1px solid rgba(0,0,0,0.08)',
            }}
          >
            <Stars />
            <div className="flex items-baseline gap-2 mt-4">
              <span
                className="font-black"
                style={{ fontFamily: 'DM Mono, monospace', fontSize: '2.5rem', letterSpacing: '-0.04em', color: '#1a1a1a' }}
              >
                4.8
              </span>
              <span
                className="text-xs font-bold uppercase tracking-widest"
                style={{ fontFamily: 'Space Grotesk, sans-serif', color: 'rgba(26,26,26,0.4)' }}
              >
                / 5.0
              </span>
            </div>
            <p
              className="text-[10px] font-bold uppercase tracking-[0.2em] mt-1"
              style={{ fontFamily: 'Space Grotesk, sans-serif', color: 'rgba(26,26,26,0.4)' }}
            >
              Durchschnittliche Gesamtbewertung
            </p>
          </div>
        </motion.div>

        {/* Testimonial cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <motion.div
              key={t.name}
              className="customer-card rounded-3xl p-5 md:p-8 reveal"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-40px' }}
              transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1], delay: i * 0.1 }}
            >
              <div className="flex items-center gap-4 mb-6">
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center font-black text-lg"
                  style={{
                    background: 'rgba(245,176,64,0.08)',
                    border: '1px solid rgba(245,176,64,0.15)',
                    color: '#f5b040',
                    fontFamily: 'DM Sans, sans-serif',
                  }}
                >
                  {t.initial}
                </div>
                <div>
                  <p className="font-bold text-sm" style={{ fontFamily: 'DM Sans, sans-serif', color: '#1a1a1a' }}>
                    {t.name}
                  </p>
                  <p
                    className="text-[9px] uppercase tracking-widest font-bold mt-0.5"
                    style={{ fontFamily: 'Space Grotesk, sans-serif', color: 'rgba(26,26,26,0.4)' }}
                  >
                    {t.location}
                  </p>
                </div>
              </div>

              <Stars count={t.rating} />

              <p
                className="text-sm italic leading-relaxed mt-4"
                style={{ color: 'rgba(26,26,26,0.6)', lineHeight: 1.7 }}
              >
                {t.text}
              </p>
            </motion.div>
          ))}
        </div>

        {/* CTA Banner */}
        <motion.div
          className="mt-12 md:mt-20 reveal"
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.9 }}
        >
          <div
            className="relative rounded-3xl p-7 md:p-16 overflow-hidden text-center"
            style={{
              background: 'rgba(0,0,0,0.02)',
              border: '1px solid rgba(0,0,0,0.07)',
            }}
          >
            {/* Decorative glow */}
            <div
              className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full"
              style={{ background: 'radial-gradient(circle, rgba(245,176,64,0.06) 0%, transparent 70%)' }}
            />

            <span
              className="text-[10px] font-bold uppercase tracking-[0.3em]"
              style={{ fontFamily: 'Space Grotesk, sans-serif', color: '#f5b040' }}
            >
              Bereit loszulegen?
            </span>
            <h2
              className="mt-4 mb-6 leading-tight"
              style={{
                fontFamily: 'DM Sans, sans-serif',
                fontWeight: 700,
                fontSize: 'clamp(1.8rem, 4vw, 3.5rem)',
                letterSpacing: '-0.04em',
                color: '#1a1a1a',
              }}
            >
              Ihre Energiezukunft
              <br />
              <span
                style={{
                  fontFamily: 'Instrument Serif, Arial, sans-serif',
                  fontStyle: 'italic',
                  fontWeight: 400,
                  background: 'linear-gradient(135deg, #f5b040 0%, #e07018 100%)',
                  WebkitBackgroundClip: 'text',
                  WebkitTextFillColor: 'transparent',
                  backgroundClip: 'text',
                }}
              >
                beginnt heute.
              </span>
            </h2>
            <p
              className="text-base max-w-lg mx-auto mb-7 md:mb-10"
              style={{ color: 'rgba(26,26,26,0.6)', lineHeight: 1.7 }}
            >
              Kostenlose Beratung, transparente Angebote, professionelle Installation —
              alles aus einer Hand.
            </p>
            <motion.button
              className="cta-pill inline-flex items-center gap-3 rounded-full px-7 py-4 md:px-10 md:py-5 font-black text-sm uppercase tracking-wider"
              style={{
                background: '#f5900a',
                color: '#ffffff',
                fontFamily: 'Space Grotesk, sans-serif',
                letterSpacing: '0.12em',
                fontSize: '13px',
              }}
              onClick={onAngebot}
              whileHover={{ scale: 1.06, y: -4 }}
              whileTap={{ scale: 0.97 }}
            >
              <span className="cta-icon"><span className="material-symbols-outlined icon-filled" style={{fontSize:"18px"}}>calculate</span></span>
              Kostenlosen Solarplan erstellen
            </motion.button>
          </div>
        </motion.div>
      </div>
    </section>
  )
}
